...
#数据库表
DRIVER_TABLES = models.sql(models.Tables())


class generateData():
    def __init__(self, db_path, xls_path=''):
        self.xls_path = xls_path
        self.dbm = self.get_dbm(db_path)  #sqlite.DBManager(db_path)
    ...
    def get_dbm(self, db_path):
        return sqlite.DBManager(db_path)

    def init_table(self, sql_list):
        # 初始化，建表
    ...
    def init_testcase(self, testcase):
        ...
        t = ('testcase_name', testcase[-1], 'usename', 'priority', 001, 'desc', testcase[3], 1, 1, 'tag', 'abs',
